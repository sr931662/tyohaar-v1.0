// IOSFrame.jsx — iOS 26 (Liquid Glass) device frame. ES module conversion of ios-frame.jsx.
// @ds-adherence-ignore -- device frame scaffold uses raw elements/hex/px by design
...
export function IOSDevice({ children, width = 402, height = 874, dark = false, title, keyboard = false }) {
  const islandW = Math.min(126, Math.round(width * 0.323));
  return (
    <div style={{
      ...
      {/* dynamic island */}
      <div style={{ position: 'absolute', top: 11, left: '50%', transform: 'translateX(-50%)', width: islandW, height: 37, borderRadius: 24, background: '#000', zIndex: 50 }} />
      {/* status bar */}
...
export function IOSKeyboard({ dark = false }) {
  ...
  return (
    <div style={{ position: 'relative', zIndex: 15, borderRadius: 27, overflow: 'hidden', padding: '11px 0 2px', display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: dark ? '0 -2px 20px rgba(0,0,0,0.09)' : '0 -1px 6px rgba(0,0,0,0.018)' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, borderRadius: 27, boxShadow: dark ? 'inset 1.5px 1.5px 1px rgba(255,255,255,0.15)' : 'inset 1.5px 1.5px 1px rgba(255,255,255,0.7)', border: dark ? '0.5px solid rgba(255,255,255,0.15)' : '0.5px solid rgba(0,0,0,0.06)', pointerEvents: 'none' }} />
      <div style={{ display: 'flex', gap: 20, alignItems: 'center', padding: '8px 22px 13px', width: '100%', boxSizing: 'border-box', position: 'relative' }}>
        {['"The"', 'the', 'to'].map((w, i) => (
          <React.Fragment key={i}>
            ...
            <div style={{ flex: 1, textAlign: 'center', fontFamily: '-apple-system, system-ui', fontSize: 17, color: sugg, letterSpacing: -0.43, lineHeight: '22px' }}>{w}</div>
          </React.Fragment>
        ))}
      </div>
